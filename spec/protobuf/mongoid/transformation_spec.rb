...
describe Protobuf::Mongoid::Transformation do
  ...
  describe "._protobuf_convert_fields_to_attributes" do
    context "when the given field's corresponding field type is :date" do
      let(:date) { Date.current }
      let(:value) { date.to_time.to_i }

      before {
        allow(User).to receive(:_protobuf_date_datetime_time_or_timestamp_field?).and_return(true)
        allow(User).to receive(:_protobuf_date_field?).and_return(true)
      }
    ...
    context "when given field's corresponding the field type is :datetime" do
      let(:datetime) { DateTime.current }
      let(:value) { datetime.to_i }

      before {
        allow(User).to receive(:_protobuf_date_datetime_time_or_timestamp_field?).and_return(true)
        allow(User).to receive(:_protobuf_datetime_field?).and_return(true)
      }
    ...
    context "when given field's corresponding the field type is :time" do
      let(:time) { Time.current }
      let(:value) { time.to_i }

      before {
        allow(User).to receive(:_protobuf_date_datetime_time_or_timestamp_field?).and_return(true)
        allow(User).to receive(:_protobuf_time_field?).and_return(true)
      }
    ...
    context "when given field's corresponding the field type is :timestamp" do
      let(:time) { Time.current }
      let(:value) { time.to_i }

      before {
        allow(User).to receive(:_protobuf_date_datetime_time_or_timestamp_field?).and_return(true)
        allow(User).to receive(:_protobuf_timestamp_field?).and_return(true)
      }
  ...
  describe ".attributes_from_proto" do
    ...
    let(:transformer) { ::Protobuf::Mongoid::Transformer.new(callable) }
    ...
    context "when a transformer is a callable that returns nil" do
      ...
      it "does not set the attribute" do
        attribute_fields = User.attributes_from_proto(proto)
        expect(attribute_fields).to match hash_including(user_attributes)
      end
    end

    context "when the transformer has a nullify_on option" do
      let(:callable) { lambda { |_proto| nil } }
      let(:transformer) { ::Protobuf::Mongoid::Transformer.new(callable, :nullify_on => :account_id) }
    ...
    context "when a transformer is a callable that returns a value" do
      ...
      it "sets the attribute" do
        attribute_fields = User.attributes_from_proto(proto)
        expect(attribute_fields).to match hash_including(user_attributes.merge(:account_id => 1))
      end
    end
  ...
  describe ".attribute_from_proto" do
    ...
    context "when the given transformer is not callable" do
      it "raises an exception" do
        expect { User.attribute_from_proto :name, nil }.to raise_exception(Protobuf::Mongoid::AttributeTransformerError)
      end
    end
  ...
  # TODO: Fix this test
  # Failling because the method attribute_from_proto is calling 
  # a method that is not defined in the Mongoid class
  # generating an deep stack trace.
  describe "#attributes_from_proto" do
    xit "gets attributes from the given protobuf message" do
