...
# Define constants outside of the block
...
describe Protobuf::Mongoid::Scope do
  before do
  ...
  describe '.search_scope' do
    ...
    it 'is chainable' do
      expect(User.limit(1).search_scope(request).order(:email.asc)).to eq User.limit(1).order(:email.asc)
    end
  ...
  describe '.upsert_key' do
    ...
    context 'no field_scope defined' do
      it 'raises an error' do
        expect { ::User.upsert_key(:foobar) }.to raise_error(::Protobuf::Mongoid::UpsertScopeError)
      end
    end
  end
  ...
  describe '.for_upsert' do
    ...
    context 'no matching upsert keys' do
      ...
      it 'raises an error' do
        expect { ::User.for_upsert(proto) }.to raise_error(::Protobuf::Mongoid::UpsertNotFoundError)
      end
    end
